...
router.get('/:id', async (req, res) => {
    const { id } = req.params;
    try {
        const [cpu] = await db.query('SELECT cpu.id, manufacturer.name, cpu_series.name AS series, cpu_microarchitecture.name AS microarchitecture, cpu_socket.name AS socket, cpu.core_count, cpu.thread_count, cpu.core_clock, cpu.boost_core_clock, cpu.L2_cache, cpu.L3_cache, cpu.tdp, integrated_graphics.model AS integrated_graphics, cpu.price from cpu JOIN cpu_series ON cpu_series.id = cpu.series_id JOIN cpu_microarchitecture ON cpu_microarchitecture.id = cpu.microarchitecture_id JOIN cpu_socket ON cpu_socket.id = cpu.socket_id LEFT JOIN integrated_graphics ON integrated_graphics.id = cpu.integrated_gpu_id JOIN manufacturer ON cpu_series.manufacturer_id = manufacturer.id WHERE cpu.id = ?', [id]);
