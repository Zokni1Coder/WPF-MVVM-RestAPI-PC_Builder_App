...
router.get('/', async (req, res) => {
    try {
        const [pss] = await db.query('SELECT ps.id, ps.model, manufacturer.name AS manufacturer, power_supply_types.type, power_supply_ratings.rating, ps.wattage, power_supply_modularity.modularity, ps.wattage, ps.price FROM power_supply AS ps JOIN manufacturer ON manufacturer.id = ps.manufacturer JOIN power_supply_types ON power_supply_types.id = ps.type JOIN power_supply_ratings ON power_supply_ratings.id = ps.efficiency_rating JOIN power_supply_modularity ON power_supply_modularity.id = ps.modularity');
