...
router.get('/', async (req, res) => {
    try {
        ...
        const [m2s] =  await db.query('SELECT motherboard, rom_form_factor.form_factor FROM `motherboard_m2type_connection` JOIN rom_form_factor ON rom_form_factor.id = m2_form');
...
router.get('/:id', async (req, res) => {
    ...
    try {
        ...
        const [m2s] =  await db.query('SELECT motherboard, rom_form_factor.form_factor FROM `motherboard_m2type_connection` JOIN rom_form_factor ON rom_form_factor.id = m2_form where motherboard = ?', [id]);
